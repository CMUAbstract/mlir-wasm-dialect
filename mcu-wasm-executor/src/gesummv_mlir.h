...
static void *app_instance_main(wasm_module_inst_t module_inst,
  ...
  TensorData arg3_tensor =
      initialize_tensor(module_inst, exec_env,
                        /*tensor_size=*/SIZE * SIZE, data2d);
  TensorData arg4_tensor = initialize_tensor(module_inst, exec_env,
                                             /*tensor_size=*/SIZE, data1d);
  TensorData arg5_tensor = initialize_tensor(module_inst, exec_env,
                                             /*tensor_size=*/SIZE, data1d);
  TensorData arg6_tensor = initialize_tensor(module_inst, exec_env,
                                             /*tensor_size=*/SIZE, data1d);
  ...
  for (int i = 0; i < 10; i++) {
    printk("%d: %f\n", i, (double)arg6_tensor.tensor_native_ptr[i]);
  }
