...
static void *app_instance_main(wasm_module_inst_t module_inst,
  ...
  InputData arg0_input = initialize_input(module_inst, exec_env,
                                          /*tensor_size=*/SIZE * SIZE,
                                          /*dim=*/2, (uint32_t[]){SIZE, SIZE},
                                          (uint32_t[]){SIZE, 1}, data2d);
  InputData arg1_input = initialize_input(module_inst, exec_env,
                                          /*tensor_size=*/SIZE * SIZE,
                                          /*dim=*/2, (uint32_t[]){SIZE, SIZE},
                                          (uint32_t[]){SIZE, 1}, data2d);
  ...
  InputData arg4_input = initialize_input(module_inst, exec_env,
                                          ...
                                          /*dim=*/2, (uint32_t[]){SIZE, SIZE},
                                          (uint32_t[]){SIZE, 1}, data2d);
  InputData arg5_input = initialize_input(module_inst, exec_env,
                                          /*tensor_size=*/SIZE * SIZE,
                                          /*dim=*/2, (uint32_t[]){SIZE, SIZE},
                                          (uint32_t[]){SIZE, 1}, data2d);
  InputData arg6_input = initialize_input(module_inst, exec_env,
                                          /*tensor_size=*/SIZE * SIZE,
                                          /*dim=*/2, (uint32_t[]){SIZE, SIZE},
                                          (uint32_t[]){SIZE, 1}, data2d);
  // load main
  ...
  // setup arguments
  argv[0] = arg0_input.input_ptr;
  argv[1] = arg1_input.input_ptr;
  argv[2] = arg2_input.input_ptr;
  argv[3] = arg3_input.input_ptr;
  argv[4] = arg4_input.input_ptr;
  argv[5] = arg5_input.input_ptr;
  argv[6] = arg6_input.input_ptr;

  // call main
  am_hal_gpio_state_write(22, AM_HAL_GPIO_OUTPUT_TOGGLE);
  if (!wasm_runtime_call_wasm(exec_env, main_func, 7, argv)) {
  ...
  for (int i = 0; i < 10; i++) {
    printk("%d: %f\n", i, (double)arg6_input.tensor_native_ptr[i]);
  }
