...
static void *app_instance_main(wasm_module_inst_t module_inst,
  ...
  TensorData arg2_data = initialize_tensor(module_inst, exec_env,
                                           /*tensor size*/ SIZE * SIZE, data2d);
  TensorData arg3_data = initialize_tensor(module_inst, exec_env,
                                           /*tensor size*/ SIZE * SIZE, data2d);
  TensorData arg4_data = initialize_tensor(module_inst, exec_env,
                                           /*tensor size*/ SIZE * SIZE, data2d);
  ...
  argv[3] = arg3_data.tensor_ptr;
  argv[4] = arg4_data.tensor_ptr;
  ...
  for (int i = 0; i < 10; i++) {
    printk("%d: %f\n", i, (double)arg2_data.tensor_native_ptr[i]);
  }
