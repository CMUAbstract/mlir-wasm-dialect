...
def add_category_labels(fig, ax, group_positions, category_labels):
    ...
    label_ax = fig.add_axes(
        [main_pos.x0, main_pos.y0 - 0.25, main_pos.width, label_height]
    )
    label_ax.axis("off")
    ...
    for pos, label in zip(group_positions, category_labels):
        label_ax.text(
            pos, 0.5, label, ha="center", va="center", fontsize=18, fontweight="bold"
        )
...
def plot_speedup(data, use_aot, binaryen_opt_level, output_file=None, title=None):
    ...
    # Ticks
    y_ticks = np.arange(np.floor(y_min * 10) / 10, np.ceil(y_max * 10) / 10 + 0.01, 0.1)
    ...
    ax.legend(
        handles=legend_elements, loc="best", frameon=True, framealpha=0.95, fontsize=18
    )
