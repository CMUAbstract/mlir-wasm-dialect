  ...
  affine.for %i = 0 to 256 {
    affine.for %j = 0 to 256 {
      ...
      affine.for %k = 0 to #map0(%i) {
        ...
        %1 = affine.load %B[%i, %j] : memref<256x256xf32>
        %2 = affine.load %C[%k, %j] : memref<256x256xf32>
        %3 = arith.mulf %alpha, %1 : f32
        %4 = arith.mulf %0, %3 : f32
        %5 = arith.addf %2, %4 : f32
        affine.store %5, %C[%k, %j] : memref<256x256xf32>
        
        %6 = affine.load %temp2[] : memref<f32>
        %7 = affine.load %B[%k, %j] : memref<256x256xf32>
      ...
      %0 = affine.load %A[%i, %i] : memref<256x256xf32>
      %1 = affine.load %B[%i, %j] : memref<256x256xf32>
      %2 = affine.load %C[%i, %j] : memref<256x256xf32>
      ...
      affine.store %9, %C[%i, %j] : memref<256x256xf32>
    }
  }
  return
}
