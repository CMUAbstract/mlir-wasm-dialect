...
AddIOpLowering::matchAndRewrite(arith::AddIOp op, OpAdaptor adaptor,
  ...
  auto lhsCastOp =
      rewriter.create<UnrealizedConversionCastOp>(op->getLoc(), localType, lhs);
  rewriter.create<wasm::TempLocalGetOp>(op->getLoc(), lhsCastOp.getResult(0));
  auto rhsCastOp =
      rewriter.create<UnrealizedConversionCastOp>(op->getLoc(), localType, rhs);
  ...
  rewriter.create<wasm::TempLocalSetOp>(op->getLoc(), tempLocalOp.getResult());
