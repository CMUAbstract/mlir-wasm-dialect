        ...
        with open(filename, "r") as file:
            ...
            for line in lines:
                try:
                    ...
                    match = re.search(
                        r"\[execution time\] (\d+\.\d+) miliseconds", stdout
                    )
                    if match:
...
def filter_and_prepare_data(data, use_aot, binaryen_opt_level):
    ...
    try:
        ...
        print(
            f"Error: binaryen_opt_level must be an integer, got: {binaryen_opt_level}",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    filtered_data = [
        d
        for d in data
        if d["use_aot"] == use_aot and d["binaryen_opt_level"] == binaryen_opt_level
    ]
...
def prepare_plot_data_transposed(data):
    ...
    for category in categories:
        ...
        category_speedups = [
            categorized_data[category][b]["speedup"] for b in benchmarks
        ]
        ...
        category_positions.append(
            position + (len(benchmarks) * within_category_spacing / 2)
        )
        category_benchmark_counts.append(len(benchmarks))

        # Move to next category position with a gap
        position += (
            len(benchmarks) * within_category_spacing
        ) + between_category_spacing / 2.0
    ...
    for i, (benchmarks, positions, speedups) in enumerate(
        zip(
            benchmarks_by_category,
            benchmark_positions_by_category,
            speedups_by_category,
        ...
        for j, (benchmark, position, speedup) in enumerate(
            zip(benchmarks, positions, speedups)
        ):
...
def plot_speedup_transposed(
    data, use_aot, binaryen_opt_level, output_file=None, title=None
):
    ...
    # X-ticks
    x_ticks = np.arange(
        np.floor(x_min * 10) / 10, np.ceil(x_max * 10) / 10 + 0.01, 0.05
    )
    ...
    for i in range(len(category_boundaries) - 1):
        ...
        if i % 2 == 0:
            ax.axhspan(
                category_boundaries[i],
                category_boundaries[i + 1],
                facecolor="lightgray",
                alpha=0.2,
                zorder=-1,
            )
    ...
    ax.axhline(
        y=category_boundaries[-1], color="gray", linestyle="-", alpha=0.7, linewidth=1.5
    )
...
def plot_speedup_side_by_side(data, binaryen_opt_level, output_file=None, title=None):
    ...
    if not interpreter_data and not aot_data:
        ...
        print(
            "Warning: Missing data for interpreter mode. Creating single plot for AOT mode."
        )
        plot_speedup_transposed(aot_data, True, binaryen_opt_level, output_file, title)
        return
    elif not aot_data:
        print(
            "Warning: Missing data for AOT mode. Creating single plot for interpreter mode."
        )
        plot_speedup_transposed(
            interpreter_data, False, binaryen_opt_level, output_file, title
        )
        return
    ...
    fig_height = max(
        12,
        max(
            len(interpreter_plot_data["all_benchmarks"]),
            len(aot_plot_data["all_benchmarks"]),
        )
        * 0.4,
    )
...
def plot_subplot(ax, plot_data, speedup_positive, speedup_negative):
    ...
    x_ticks = np.arange(np.floor(x_min * 10) / 10, np.ceil(x_max * 10) / 10 + 0.01, 0.1)
    ax.set_xticks(x_ticks)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot LLVM vs WAMI performance comparison grouped by category"
    )
    ...
    plot_speedup_side_by_side(
        data, args.binaryen_opt_level, output_file=args.output, title=args.title
    )
